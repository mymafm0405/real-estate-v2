...
export class ClientsService {
  ...
  addClient(newClient: Client) {
    this.generalService.addNewData('clients', newClient).subscribe(
      (res: { name: string }) => {
  ...
  checkClientExist(qId: number, newClient: Client) {
    if (this.clients.find((client) => client.qId === qId)) {
      this.newClientIdChanged.next(
        this.clients.find((client) => client.qId === qId).id
      );
    } else {
  ...
  loadClients() {
    this.generalService.getArrayOfData('clients').subscribe(
      (res: Client[]) => {
        this.clients = res;
        this.clientsChanged.next(true);
      },
