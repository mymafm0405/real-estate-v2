...
export class ContractsService {
  ...
  constructor(private generalService: GeneralService, private unitsService: UnitsService) {}
  ...
  addContract(newContract: Contract) {
    this.generalService.addNewData('contracts', newContract)
    .subscribe(
      (res: {name: string}) => {
        this.contracts.push({...newContract, id: res.name});
        this.contractsChanged.next(true);
        this.contractAddingStatus.next(true);
        this.unitsService.changeUnitContractId(newContract.unitId, res.name);
      }, error => {
        console.log(error);
  ...
  loadContracts() {
    this.generalService.getArrayOfData('contracts')
    .subscribe(
      (res: Contract[]) => {
        this.contracts = res;
        this.contractsChanged.next(true);
